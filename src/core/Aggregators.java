...
public final class Aggregators {
  ...
  /** Aggregator that correlates data points with itself. */
  public static final Aggregator CORR = new Corr(Interpolation.LERP, "corr");
  public static final Aggregator FOURIER = new Fourier(Interpolation.ZIM, "fourier");
  /**
  ...
  static {
    ...
    //custom aggregator
    aggregators.put("corr", CORR);
    aggregators.put("fourier", FOURIER);
  ...
  private static final class Corr extends Aggregator {
    public Corr(final Interpolation method, final String name) {
      super(method, name);
    }
    ...
    public long runLong(final Longs values) {
      ...
      long sxy = 0L;
      while (values.hasNextValue()) {
        long currX = values.nextLongValue();
        long currY = values.nextLongValue();
        sx += currX;
        sy += currY;
        sxx += currX * currX;
        syy += currY * currY;
      ...
      result = covariation / sigmaX / sigmaY;
  ...
  private static final class Fourier extends Aggregator {
    // DFT with 65536 samples and sample rate 131072
    private static final int SAMPLE_RATE = 262144;

    public Fourier(final Interpolation method, final String name) {
      super(method, name);
    }

    private Complex[] toComplexArray(List<Complex> values) {
    ...
    public long runLong(final Longs values) {
        ...
        List<Complex> collection = Lists.newArrayList();
        while (values.hasNextValue()) {
          collection.add(new Complex(((double) values.nextLongValue())));
        }
        Complex[] fftResult = fft.transform(prefft(toComplexArray(collection)), TransformType.FORWARD);
        long spectrumValuesPerTimestamp = 0L;
        ...
        for (int i = 0; i < fftResult.length; i++) {
            ...
            spectrumValuesPerTimestamp += (long) Math.sqrt((rr * rr) + (ri * ri));
        }
        return spectrumValuesPerTimestamp;
    }
    public static Complex[] prefft(Complex[] x) {
        ...
        int i = 0;
        while (b) {
            //yesli kolichestvo elementov v massive ravno stepeni 2 to vozvrashaem ego
            if (N == Math.pow(2,++i)) return x;
            // inache vihodim is cikla
            else if (N < Math.pow(2 ,i)) b = false ;
        }
        // sozdaem novyi massiv s kolichestvom elementov ravnym stepeni 2
...
//      double[] spectrumValues = new double[collection.size()];
      double spectrumValuesPerTimestamp = 0;
      //вычисляем модуль квадратов действительной и мнимой части для построения амплитудного спектра
      for (int i = 0; i < fftResult.length; i++) {
        double rr = (fftResult[i].getReal());
        double ri = (fftResult[i].getImaginary());
        spectrumValuesPerTimestamp += Math.sqrt((rr * rr) + (ri * ri));
      }
      return spectrumValuesPerTimestamp;
    }
  }
